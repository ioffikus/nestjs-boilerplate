...
import { ContextService } from 'src/shared/providers/context.service';
...
export class AuthService {
  ...
  async createToken(account: { id: number }): Promise<TokenPayloadDto> {
  ...
  async validateAccount(data: {
    ...
    const account = await this.accountsService.findByNameOrEmail({
      email: data.email,
    });
